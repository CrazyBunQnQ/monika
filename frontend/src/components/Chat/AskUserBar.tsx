...
function stripTrailingList(text: string, hasOptions: boolean): string {
    if (!hasOptions) return text
    const lines = text.split('\n')
    while (lines.length > 0 && lines[lines.length - 1].trim() === '') lines.pop()
    const listPattern = /^\s*(?:\d+[.)]\s+|[-*•]\s+)/
    while (lines.length > 0 && listPattern.test(lines[lines.length - 1])) lines.pop()
    while (lines.length > 0 && lines[lines.length - 1].trim() === '') lines.pop()
    return lines.join('\n')
}
...
function AskUserBar({ sessionId }: AskUserBarProps) {
    ...
    const cleanQuestion = useMemo(
        () => stripTrailingList(pendingAskUser.question, !!hasOptions),
        [pendingAskUser.question, hasOptions],
    )
        ...
        >
            <div className="flex items-center gap-1.5">
                <svg width="12" height="12" viewBox="0 0 16 16" fill="none" style={{ color: 'var(--accent)', flexShrink: 0 }}>
                    ...
                    <path d="M6.5 6.5C6.5 5.5 7.2 5 8 5C8.8 5 9.5 5.5 9.5 6.5C9.5 7.5 8 7.5 8 8.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
            ...
            <div className="text-[13px]" style={{ color: 'var(--text-primary)' }}>
                <MarkdownBlock content={cleanQuestion} />
            </div>
