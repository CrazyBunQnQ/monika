...
function ChatArea(props: IDockviewPanelProps) {
    ...
    const handleRunShell = async (command: string) => {
        ...
        store.addShellExecutingSession(sessionId)

        try {
            App.RunShellCommand(projectPath, sessionId, command)
        } catch (err) {
            store.removeShellExecutingSession(sessionId)
            store.appendToSession(sessionId, [
                { id: crypto.randomUUID(), role: 'shell' as const, content: `$ ${command}\nError: ${String(err)}` },
            ])
        }
    }
