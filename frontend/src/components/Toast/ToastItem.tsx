...
export function ToastItem({ item, onDismiss }: ToastItemProps) {
    ...
    const typeLabel = item.type === 'reply-complete'
        ? '回复完成'
        : item.type === 'permission-request'
            ? '需要确认'
            : item.operation === 'success'
                ? 'Git'
    ...
    return (
      ...
      `}
            style={item.type === 'operation' ? { borderLeft: `3px solid ${accentColor}` } : undefined}
        >
            {item.type === 'operation' ? (
                <div className="flex items-start gap-2">
                    <div className="text-[12px] text-[var(--text-primary)] flex-1 break-words">{item.message}</div>
                    <button
                        onClick={handleDismiss}
                        className="text-[11px] text-[var(--text-dim)] hover:text-[var(--text-primary)] shrink-0"
                    >
                        ×
                    </button>
                </div>
